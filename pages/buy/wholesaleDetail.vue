<template>
	<view class="wholesaleDetail">
		<view class="FY FY-c FX-c" v-if="nodata" style="font-size: 16px;height: calc(80vh);">
			<tui-icon name="nodata" :size="60" color="#999"></tui-icon>
			暂无内容
		</view>
		<scroll-view scroll-y class="wholesaleDetail_content" v-if="!!!nodata">
			<view class="entrusbuylist_1" v-for="item in wholesaleDetailList">
				...
				<view class="entrusbuylist_1_center" style="flex: 1;">
					...
					<view class="entrusbuylist_1_right_price">
						...
						<text>数量:{{item.ut_num - item.ut_picknum}}份</text>
...
<script>
	...
	export default {
		...
		onLoad: async function() {
			const pflist_res = await this.pflist();
			// 请求是否成功
			if (pflist_res.status !== 200) { this.nodata = true; this.showToast(pflist_res.msg); return };
			// 请求到的数据是否为null
			if (!!!pflist_res.data) { this.nodata = true; return };
			// 请求到的数据是否空数组
			if (pflist_res.data.length === 0) { this.nodata = true; return };
			this.wholesaleDetailList = pflist_res.data;
		},
		methods: {
			// 信息反馈
			showToast: function(data) { uni.showToast({ title: data, icon: "none", mask: true }) },
			// 列表请求
			pflist: async function() {
