				...
				<tui-card v-if="isSuccess" class="tui-card" :image="bottomList.img[0].img" :title="bottomList.img[0].title" :tag="bottomList.img[0].tag"
				 @click="popup">
					<template v-slot:footer>
						...
						<view class="tui-tips">
							<tui-icon name="circle-fill" :size="30" :color="'#9E2036'"></tui-icon>
						</view>
					</template>
				</tui-card>
				<view class="title_2" v-if="isSuccess">
				...
				<view class="form_information" v-if="isSuccess">
					<form @submit="formSubmit">
						...
						<tui-list-cell :hover="false">
							<view class="tui-line-cell">
								<view class="tui-title">卖出价格：</view>
								<input v-model="price" placeholder-class="tui-phcolor" class="tui-input" name="amount" placeholder="请输入卖出价格"
								 maxlength="50" type="digit" />
							</view>
						</tui-list-cell>
						<tui-list-cell :hover="false">
							<view class="tui-line-cell">
								...
								<input v-model="num" placeholder-class="tui-phcolor" class="tui-input" name="amount2" placeholder="请输入卖出数量"
								 maxlength="50" type="digit" />
							</view>
						</tui-list-cell>
							...
							<scroll-view scroll-y class="tui-share-content">
								<view class="share-content-list" v-for="(item, index) in bottomLists" :key="index" @click="clickBottomList(index)">
									...
									<label class="tui-checkbox">
										<tui-icon name="circle" :size="30" :color="'#9E2036'" @click="popup" v-if="index !== current"></tui-icon>
										<tui-icon name="circle-fill" :size="30" :color="'#9E2036'" @click="popup" v-if="index === current"></tui-icon>
									</label>
								</view>
							</scroll-view>
						</radio-group>
		...
		<scroll-view scroll-y="true" v-if="currentTab==1" class="right_delisting">
			...
			<view class="zhaipai_content" v-for="(v,i) in detail" :key="i" v-if="debool==true">
				<buylist :imgUrl="v.g_pic" :code="v.g_code" :title="v.g_title" :price="v.ut_price" :num="v.stock_num" g_price="卖出价:￥"
				 g_num="数量:" button="卖出" @click="showModal(v.g_id,v.ut_id)"></buylist>
			</view>
					...
					uni.getStorage({
						...
						success: function(res) {
							...
							if (that.user != 1) {
								...
								that.sendRequest({
									url: App.consignmentSale,
									method: 'POST',
									data: datas,
									success: (res) => {
										console.log("确认委托卖出", res)
...
<style lang="scss" scoped>
	...
	/deep/ .tui-green-outline {
		...
		border: 2rpx solid #9E2036 !important;
	}
